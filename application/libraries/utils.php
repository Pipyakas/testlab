...
class utils {
	...
	public function view_counter($products_id) {
		$cookie_name = 'products_' . $products_id;
		$CI = &get_instance();
		if (!isset($_COOKIE[$cookie_name])) {
			$CI -> load -> model('products_model');
			$data = $CI -> products_model -> get_by_id($products_id);
			$count_of_views = $data[0] -> view_counter;
			$count_of_views += 1;
			$data_array = array('view_counter' => $count_of_views);
			$array_where = array('id' => $products_id);
			$CI -> products_model -> update($data_array, $array_where);
			$CI -> input -> set_cookie($cookie_name, $products_id, time() + 3600);
		};
	}
    ...
    //render frontend view
		static function _render_frontend($content='tmp',$data=null){//view/tmp.php
			...
			$CI->template
			->set_partial('header','frontends/commons/header')
	    ...
	    static function _render_frontend_no_slide($content='tmp',$data=null){//view/tmp.php
	    	$CI =& get_instance();
	    	$CI->load->helper('settings_helper');
	    	if(!isset($data['title'])){
	    		$data['title']=SITE_NAME;
	    	...
	    	$data['general_setting']=getSettings(GENERAL_SETTING_FILE);
	    	$CI->template
	    	->set_partial('header','frontends/commons/header_no_slide')
	    	->set_partial('content', $content)
	    	->set_partial('footer','frontends/commons/footer')
			->set_layout('default') //themes/../views/layouts/default.php
			->build('tmp',$data); //views/tmp
