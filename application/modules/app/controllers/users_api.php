...
class Users_api extends REST_Controller
{
	...
	{ 
		...
		if($data!=null){
			$this->response($data); 
			//$this->response(array('empty'=>'empty_hung'));
		}else{
	...
	function access_get()
	{
		$this->response(array('empty'=>'empty_hu'));
	}
	...
	function productload_post(){
		$this->load->model('products_model');
		if(isset($_SESSION['user'])){
           $user=$_SESSION['user'][0];
        }
		$where = '`products`.`activated`=1 AND `users`.`activated`='.ACTIVATED;
		$select='SELECT * FROM products JOIN `users` ON `products`.`user_id` = `users`.`id` WHERE '.$where.'';
		$data =$this->products_model->get_by_query($select);
		if($data!=null){
					$this->response($data);
				}else{
					$this->response(array('empty'=>'empty_data'));
