    ...
    if ((isset($_POST['user_name'])) && (isset($_POST['pwd'])) && (isset($_POST['pwd2']))) {
      ...
      if ($this->form_validation->run() != false) {
        ...
        if ($data == null) {
          ...
          $result = $this->users_model->insert($insert_data); //insert data to db
        }
      }
    }
...
?>
          <?php
          $data = $this->users_model->get_by_username_and_pwd($user_name, encrypt_pwd($pwd)); //check user in db
          if ($data != null) {
          ?>
          <?php
            $_SESSION['user'] = $data;
            ...
            $this->users_model->update($updated_data, array('id' => $data[0]->id)); //insert data to db
            redirect(base_url());
          } else {
          ?>
            <script>
